...
#
# Change log
#   13/07/2020: Created this program, by modifying mainpar7.R
#
...
calcExcludesOne <- function(low,upp) {
  return(sum(low > 1 | upp < 1,na.rm = T)/length(which(!is.na(low))))
}
...
doCalculations <- function(population,phi,n,b,N) {
  ...
  for (sim in 1:N) {
    ...
    if (phi < chisq.phi.low[sim] | phi > chisq.phi.upp[sim]) {
      chisq.phi.err[sim] <-  1
    }
    ...
    if (do.RR) {
      ...
      vgam.phi.err[sim] <- (phi < phihat3ci[1]) | (phi > phihat3ci[2])
  ...
  chi.phi.p1 <- calcExcludesOne(chisq.phi.low,chisq.phi.upp)
  vga.phi.p1 <- calcExcludesOne(vgam.phi.low,vgam.phi.upp)
  
  result.prop1 <- c(chi.p1,vga.p1,chi.phi.p1,vga.phi.p1)
  ...
  result.coverage <- c(1-errorChisq,1-errorVGAM,errorChisq.phi,errorVGAM.phi)
  ...
  result.width <- c(chi.width,chi.width.phi,vga.width,vga.width.phi)
...
write.csv(results,'~/results-mainpar11.csv',row.names=F,quote=F)
print('finished')
print(Sys.time())
