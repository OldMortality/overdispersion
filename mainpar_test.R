...
#profvis({
#  t <- doCalculations(population,phi,n,b,300)
#})


# computes everything. Will return a vector of 18:
#   6 error rates, 6 proportions, and 6 median widths
#
#
doCalculations <- function(population,phi,n,b,N) {
  ...
  for (sim in 1:N) {
    ...
    if (do.RR) {
      ...
      tryCatch(
        {
          # another simulation phi = 1
          
          y<-rpois(n,mu) 
          # another simulation phi = 2
          phi = 2
          nu <- phi-1
          mn<-mu/nu
          y<-rnegbin(n,mu,mn) 
  ...
  ga
  m.width <- calcMedianWidth(gamma.low,gamma.upp)
